...
# 修复：固定使用 us-east5，该 region 对 Claude 模型支持最稳定
...
if not PROJECT_ID:
    st.error("❌ 未检测到 GOOGLE_CLOUD_PROJECT，请在 Streamlit Cloud Secrets 中配置。")
    st.stop()
...
# claude-3-5-sonnet-v2 quota 更宽松，作为首选
# 如需更新可换为 claude-sonnet-4-5@20251205
MODEL = "claude-3-5-sonnet-v2@20241022"

# ── 带重试的 API 调用函数 ───────────────────────────────────
def call_claude(messages, system_prompt, max_tokens, temperature, retries=3):
    """
    调用 Claude，遇到 429 quota 错误时自动重试（指数退避）。
    """
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Claude 思考中..."):
            ...
            # 取最近 10 条（节省 token），并过滤掉开头的 assistant 消息
